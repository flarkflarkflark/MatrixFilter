...
- [ ] All `.h` files
  - [ ] Include guards (e.g., `#ifndef MATRIX_FLANGER_H`)
  - [ ] Class declarations
  - [ ] Namespace declarations
  - [ ] Comments
  - [ ] Constants/defines
...
Old format: `#ifndef MATRIX_FLANGER_H`
...
Old: `namespace MatrixFlanger { ... }`
New: `namespace MatrixFilter { ... }`

Check all:
- [ ] Namespace declarations
- [ ] Namespace usages (`MatrixFlanger::SomeClass`)
- [ ] Using statements (`using namespace MatrixFlanger;`)
...
# Look for any remaining "flanger" references
git diff | grep -i flanger

# Check all tracked files
git ls-files | xargs grep -l -i "flanger"
```
...
```bash
# Find in all files
grep -r -i "flanger" . --exclude-dir={.git,build*,external}

# Find in C++ files only
find . -name "*.cpp" -o -name "*.h" -o -name "*.hpp" | xargs grep -i "flanger"

# Find in CMake files
find . -name "CMakeLists.txt" | xargs grep -i "flanger"

# Find in scripts
find . -name "*.sh" -o -name "*.bat" | xargs grep -i "flanger"
```
...
- [ ] No "flanger" references found (except in this checklist!)
